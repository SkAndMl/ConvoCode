...
if __name__ == "__main__":
    for library in os.listdir("./data"):
        path = os.path.join("./data", library)
        if os.path.isdir(path):
            if library not in os.listdir("vectorstores"):
                result = create_index(library_name=library)
                if isinstance(result, bool):
                    print(f"Index successfully created at {INDEX_PATH.format(library_name=library)}")
                else:
