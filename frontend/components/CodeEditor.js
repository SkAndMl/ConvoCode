...
const CodeEditor = () => {
  ...
  const autocomplete = async () => {
    try {
      const incomplete_code = getLastFullLine(code);
      console.log("incomplete team"+incomplete_code);
      const encodedData = {incomplete_code, user_id};
      const response = await fetch('http://127.0.0.1:8000/completeCode', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(encodedData),
      });
      const data = await response.json();
      const newCode = code.substring(0, code.lastIndexOf('\n') + 1) + data.completed_code;
      setCode(newCode); 
      console.log(data["completed_code"])
